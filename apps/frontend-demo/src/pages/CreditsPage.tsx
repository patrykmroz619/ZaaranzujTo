...
const packages: CreditPackage[] = [
  ...
  { id: 'standard', name: 'Standard', credits: 30, price: 49.99, popular: true },
        ...
        <div className="mx-auto grid max-w-3xl gap-6 md:grid-cols-3">
          {packages.map((pkg, i) => (
            <motion.div key={pkg.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
              <Card className={`relative transition-shadow hover:shadow-elevated ${pkg.popular ? 'border-primary shadow-elevated' : ''}`}>
                {pkg.popular && (
                ...
                <CardContent className="flex flex-col items-center p-6 pt-8">
                  ...
                  <Button className={`w-full ${pkg.popular ? 'gradient-warm text-primary-foreground border-0' : ''}`} variant={pkg.popular ? 'default' : 'outline'}>
                    {t('credits.buy')}
                  </Button>
                </CardContent>
              </Card>
