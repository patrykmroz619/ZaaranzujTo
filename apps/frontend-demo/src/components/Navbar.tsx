  ...
  return (
    <header className="sticky top-0 z-50 border-b bg-background/80 backdrop-blur-md">
      <div className="container flex h-14 items-center justify-between">
            ...
            <nav className="hidden items-center gap-1 md:flex">
              {navLinks.map((link) => (
                <Button
                  ...
                  className={isActive(link.path) ? 'bg-accent text-accent-foreground' : 'text-muted-foreground'}
                >
                  {link.label}
                </Button>
              ))}
        ...
        {isAuthenticated ? (
                ...
                <button className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-accent/50 transition-colors">
                  ...
                  {!isMobile && (
                    <div className="text-left">
                      <p className="text-sm font-medium text-foreground leading-tight">Nowy użytkownik</p>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" onClick={() => navigate('/register')} className="gradient-warm text-primary-foreground border-0">
              {t('nav.register')}
            </Button>
          </div>
        )}
      ...
      {isAuthenticated && mobileOpen && (
        <>
          <div className="fixed inset-0 top-14 z-40 bg-black/40 md:hidden" onClick={() => setMobileOpen(false)} />
